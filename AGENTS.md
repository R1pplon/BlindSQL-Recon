...
8-script linear pipeline. Directory numeric prefixes match pipe order:
...
→  url_decoder.py  →  [base64_decoder.py]  →  sqlmap_analyzer.py
   (URL-decode)         (Base64-decode, optional)   (pattern-match blind payloads)
→  default_data_reconstructor.py  →  default_report_generator.py
   (reconstruct chars via binary search)    (console + file reports)
```

## Pipeline metadata passing (critical)

`2_param_detector.py` writes **one metadata line** to stdout before all data:
...
The file is `sqlmap_analyzer.py`, **not** `3_payload_analyzer.py`. CLI: `python sqlmap_analyzer.py --config <yaml>`.

## Required args

- `sqlmap_analyzer.py`: `--config` is **required** (no default). Pre-built configs at `3_payload_analyzer/config/test_boolean_config.yaml` and `test_time_config.yaml`.
...
- `2_param_detector.py` loads `DEEPSEEK_API_KEY` from project-root `.env` or CWD `.env`.
